...
//=================================天文常数=========================================
//==================================================================================
const cs_rEar = 6378.1366; //地球赤道半径(千米)
const cs_rEarA = 0.99834 * cs_rEar; //平均半径
const cs_ba = 0.99664719; //地球极赤半径比
const cs_ba2 = cs_ba * cs_ba; //地球极赤半径比的平方
const cs_AU = 1.49597870691e8; //天文单位长度(千米)
const cs_sinP = cs_rEar / cs_AU; //sin(太阳视差)
const cs_PI = Math.asin(cs_sinP); //太阳视差
const cs_GS = 299792.458; //光速(行米/秒)
const cs_Agx = cs_AU / cs_GS / 86400 / 36525; //每天文单位的光行时间(儒略世纪)
const cs_xxHH = [116, 584, 780, 399, 378, 370, 367, 367]; //行星会合周期
const xxName = ['地球', '水星', '金星', '火星', '木星', '土星', '天王星', '海王星', '冥王星'];
const rad = 180 * 3600 / Math.PI; //每弧度的角秒数
const radd = 180 / Math.PI; //每弧度的度数
const pi2 = Math.PI * 2;
const pi_2 = Math.PI / 2;
const J2000 = 2451545;
const cs_k = 0.2725076; //月亮与地球的半径比(用于半影计算)
const cs_k2 = 0.2722810; //月亮与地球的半径比(用于本影计算)
const cs_k0 = 109.1222; //太阳与地球的半径比(对应959.64)
const cs_sMoon = cs_k * cs_rEar * 1.0000036 * rad; //用于月亮视半径计算
const cs_sMoon2 = cs_k2 * cs_rEar * 1.0000036 * rad; //用于月亮视半径计算
const cs_sSun = 959.64; //用于太阳视半径计算
//=================================三角函数等=======================================
//==================================================================================
...
function rad2strE(d, tim, ext) {
    ...
    s += a.slice(a.length - 3, a.length) + w1;
    s += b.slice(b.length - 2, b.length) + w2;
    s += c.slice(c.length - 2, c.length);
    if (ext)
        s += "." + d.slice(d.length - ext, d.length) + w3;
    return s;
}
//将弧度转为字串,保留2位
...
//日期通用類元件
class _JD_ {
    //初始化历法，calendarFlag历法标志，0为GC格里历，1为JC儒略历，2为农历
    //LCMonthLeapFlag:本月农历月是非闰月，0为不润，1为润；
    constructor(calendarFlag = 0, Year = 2000, Month = 1, Day = 1, Hour = 12, Min = 0, Sec = 0, LCLeapFlag = 0) {
        ...
        console.log('_JD_类初始化:时间数值为' + this.time.toFixed(5));
        this.Year = Year - 0; //1583/10/15之后等同于格里历GC的年月日，之前等同于儒略历JC的年月日
        this.Month = Month - 0;
        this.Day = Day - 0;
        this.GCYear = Year - 0; //格里历年
        this.GCMonth = 1;
        this.GCDay = 1;
        this.JCYear = Year - 0; //儒略历年
        this.JCMonth = 1;
        this.JCDay = 1;
        this.LCYear = Year - 0; //农历年
        this.LCMonth = 1;
        this.LCDay = 1;
        this.GCYearLeap = this.setGCYearLeap(this.GCYear); //格里历是否闰年
        this.JCYearLeap = this.setJCYearLeap(this.JCYear); //儒略历是否闰年
        this.LCMonthLeap = LCLeapFlag; //农历是是否是闰月
        this.thisJD = 2451545; //	格里历2000/1/1 12:00的JD
    }
    init_YMD2JD(calendarFlag, Year, Month, Day, LCLeapFlag = 0) {
    }
    //设置格里历GC年是否是闰年
    setGCYearLeap(GCYear) {
        let yearTemp = GCYear + 12000;
        if ((yearTemp % 4) == 0) {
            this.GCYearLeap = true;
        }
        else {
            this.GCYearLeap = false;
        }
        if ((yearTemp % 100) == 0) {
            this.GCYearLeap = false;
        }
    ...
    //设置儒略历JC年是否是闰年
    setJCYearLeap(JCYear) {
        let yearTemp = JCYear + 12000;
        if ((yearTemp % 4) == 0) {
            this.JCYearLeap = true;
        }
        else {
            this.JCYearLeap = false;
        }
        return this.JCYearLeap;
    }
    //202302031200形式的日期數據日期轉化為日期数值数组
    //分割日期時間串，返回：[公元前後標誌,年，月，日，小時，分鐘，合成小時數]
    dataTimeStrSeparate(dataTimeString) {
        let sTimeDateTemp = dataTimeString.split('.');
        //console.log('JD.dataTimeStrSeparate():'+sTimeDateTemp[0] + '/性別：'+ sTimeDateTemp[1]);
        //let vLengthTemp:number	=	sTimeDateTemp.length;
        let sFirstCharTemp = sTimeDateTemp[0].slice(0, 1);
        //let sADTemp:string	=	'';		//	公元前後的標誌
        let iADTemp = 1; //	公元前後的標誌
        let yearTemp = ''; //	年的標誌
        let monthTemp = ''; //	月的標誌
        let dayTemp = ''; //	日期的標誌
        let hourTemp = ''; //	小時的標誌
        let minuteTemp = ''; //	分鐘的標誌
        let timeTemp = ''; //	分鐘的標誌
        let genderTemp = Number(sTimeDateTemp[1].slice(0, 1)); //gender = 0，男性；gender = 1，女性，
        let hemisphereTemp = Number(sTimeDateTemp[1].slice(1, 2)); //hemisphere=0，北半球，hemisphere=1，南半球；
        if (sFirstCharTemp == '-') {
            //sADTemp	= '公元前';
            iADTemp = -1;
        }
        ...
        //console.log('JD.dataTimeStrSeparate():'+iADTemp+'/'+yearTemp+'/'+monthTemp+'/'+dayTemp+'/'+hourTemp	+'/'+minuteTemp+'/'+timeTemp+'/'+genderTemp+'/'+hemisphereTemp);
    ...
    //公历转儒略日,儒略曆1582年10月04日
    JD(year, month, day) {
        let JDTemp = this.jc_gc_Date2JD(year, month, day);
        return JDTemp;
    }
    //公历转儒略日，类中的当前的(Year,Month,Day,Hour,Min,Sec),儒略曆1582年10月04日
    toJD() {
        let JDTemp = this.jc_gc_Date2JD(this.Year, this.Month, this.Day + ((this.Sec / 60 + this.Min) / 60 + this.Hour) / 24);
        return JDTemp;
    }
    ;
    //儒略历jc/格里历GC混合在一起进行转化，年月日转儒略日,GCDay:需要帮时间转换后加上，儒略曆1582年10月04日12時00分00秒之前使用儒略历转化
    jc_gc_Date2JD(Year, Month, Day) {
        let n = 0, calendarFlag = 0;
        if ((Year * 372 + Month * 31 + int2(Day)) >= 588829)
            calendarFlag = 1; //判断是否为格里高利历日1582*372+10*31+15
        if (Month <= 2)
            Month += 12, Year--;
        if (calendarFlag)
            n = int2(Year / 100), n = 2 - n + int2(n / 4); //加百年闰
        return int2(365.25 * (Year + 4716)) + int2(30.6001 * (Month + 1)) + Day + n - 1524.5;
    }
    //格里历GC，年月日转儒略日,GCDay:需要帮时间转换后加上
    gcDate2JD(GCYear, GCMonth, GCDay) {
        ...
        let JD = Math.floor(365.25 * (GCYear + 4716)) + Math.floor(30.6001 * (GCMonth + 1)) + GCDay + B - 1524.5;
        return JD;
    }
    //儒略历JC，年月日转儒略日,JCDay:需要帮时间转换后加上
    jcDate2JD(JCYear, JCMonth, JCDay) {
        let a = Math.floor((14 - JCMonth) / 12);
        let y = JCYear + 4800 - a;
        let m = JCMonth + 12 * a - 3;
        //-0.5，是最后的调整
        let JD = JCDay + Math.floor((153 * m + 2) / 5) + 365 * y + Math.floor(y / 4) - 32083 - 0.5;
        return JD;
    }
    //取格里历年，年月日内的天数，1月1日，为0日
    getGC_DayNum(GCYear, GCMonth, GCDay) {
        let yearJD = 0;
        let isleap = this.getGCYearLeap(GCYear);
        if (GCMonth == 1) {
            yearJD = GCDay;
        }
        else if (GCMonth == 2) {
    ...
    //取儒略历年，年月日内的天数，1月1日，为0日
    getJC_DayNum(JCYear, JCMonth, JCDay) {
        let yearJD = 0;
        let isleap = this.getJCYearLeap(JCYear);
        if (JCMonth == 1) {
            yearJD = JCDay;
        }
        else if (JCMonth == 2) {
    ...
    // 儒略日转格里历（Gregorian calendar）算法,20231214新的
    JD2GCDate_2(JD) {
        let date = new Object();
    ...
    // 儒略日转儒略历（Julian calendar）算法,20231214新的
    JD2JCDate_2(JD) {
        let date = new Object();
        let Z = Math.floor(JD + 0.5);
        let A = Z + 1524;
    ...
    getGC0Date2str(r) {
        ...
        let s = int2(r.Sec + .5);
        let sStr = "";
        //console.log("JD之getGC0Date2str())1:" +hStr +"/"+ h +"/"+ mStr + "/"+ m + "/" + s+"/"+ sStr);
        let dataTimeStr1 = '';
        let dataTimeStr2 = '';
        if (s >= 60)
            s -= 60, m++;
        ...
        Y = Y.slice(Y.length - 5, Y.length); //.padStart(4, "0")
        ...
        //console.log("JD之getGC0Date2str())2:" + mStr + "/"+ m + "/" + s+"/"+ sStr);
        //return Y+"-"+M+"-"+D+" "+h+":"+m+":"+s;
        //返回：日期： 1084-12-28 00:00:00 108412280000
        dataTimeStr1 = Y + yearDelimiter + M + yearDelimiter + D + yearTimeDelimiter + hStr + timeDelimiter + mStr + timeDelimiter + sStr;
        if (Y >= 0) {
            dataTimeStr2 = Y + M + D + hStr + mStr + '.00  /  ' + Y + M + D + hStr + mStr + '.10';
        }
        else {
            dataTimeStr2 = Y + M + D + hStr + mStr + '.00  / ' + Y + M + D + hStr + mStr + '.10';
        }
        return String(dataTimeStr1 + gLCalendarBase.getNianHao(r.Year) + '       |      ' + dataTimeStr2);
    }
    ;
    //JD数值转日期字符串
    JD2str(jd) {
        let r = this.JD2GC0Date(jd);
        return this.getGC0Date2str(r);
    }
    ;
    //儒略日数转公历
    setJD2YMD(jd) {
    ...
    getTimeStr(jd) {
        ...
        m = "0" + m;
        s = "0" + s;
        return (h.slice(h.length - 2, h.length) + timeDelimiter + m.slice(m.length - 2, m.length) + timeDelimiter + s.slice(s.length - 2, s.length));
    }
    ;
    //星期相关
    //Weeks:string[] = ['日', '一', '二', '三', '四', '五', '六', '七'];
    //星期计算
    getWeek(jd) {
    ...
    //求y年m月的第n个星期w的儒略日数
    getJD4Week(y, m, n, w) {
        let jd = JD.JD(y, m, 1.5); //月首儒略日
        let w0 = (jd + 1 + 7000000) % 7; //月首的星期
    ...
    //返回特定年份的特定节气的开始和结束时间：JD；節氣数：从立春节为1，大寒气为24；
    //year:是从立春：1号節氣开始的年
    getJieQiStartEndJD(year, JieQiNo) {
        let yearFrom2k = year - 2000; //从2000为纪元0年起点
        let thisJieQi1JD1;
        let thisJieQi1JD2; //上一個節氣開始
        let thisJieQi2JD1;
        let thisJieQi2JD2; //當前節氣開始
        let thisJieQi3JD1;
        let thisJieQi3JD2; //當前節氣結束
        let thisJieQi4JD1;
        let thisJieQi4JD2; //下一個節氣結束
        //精确节气时间计算，日月星历基本函数类：已知太阳视黄经反求时间
        thisJieQi1JD1 = Sun_Moon_Ephemeris.S_aLon_t((yearFrom2k + (JieQiNo - 5) * 15 / 360 + 1) * 2 * Math.PI);
        thisJieQi1JD2 = thisJieQi1JD1 * 36525 + J2000 + 8 / 24 - dt_T(thisJieQi1JD1 * 36525); //	调整回正常的JD计算
        thisJieQi2JD1 = Sun_Moon_Ephemeris.S_aLon_t((yearFrom2k + (JieQiNo - 4) * 15 / 360 + 1) * 2 * Math.PI);
        thisJieQi2JD2 = thisJieQi2JD1 * 36525 + J2000 + 8 / 24 - dt_T(thisJieQi2JD1 * 36525); //	调整回正常的JD计算
        thisJieQi3JD1 = Sun_Moon_Ephemeris.S_aLon_t((yearFrom2k + (JieQiNo - 3) * 15 / 360 + 1) * 2 * Math.PI);
        thisJieQi3JD2 = thisJieQi3JD1 * 36525 + J2000 + 8 / 24 - dt_T(thisJieQi3JD1 * 36525); //	调整回正常的JD计算
        thisJieQi4JD1 = Sun_Moon_Ephemeris.S_aLon_t((yearFrom2k + (JieQiNo - 2) * 15 / 360 + 1) * 2 * Math.PI);
        thisJieQi4JD2 = thisJieQi4JD1 * 36525 + J2000 + 8 / 24 - dt_T(thisJieQi4JD1 * 36525); //	调整回正常的JD计算
        return [thisJieQi1JD2, thisJieQi2JD2, thisJieQi3JD2, thisJieQi4JD2];
    }
}
;
var JD = new _JD_(0, 2000, 1, 1, 13, 31, 29, 0); //初始化为格里历，2000/1/1 12:31:29 非农历闰月
//=================================岁差计算=========================================
//==================================================================================
//IAU1976岁差表
var preceTab_IAU1976 = [
...
//=============================一些天文基本问题=====================================
//==================================================================================
//返回朔日所在朔月的编号,jd应在朔月附近，允许误差数天；29.5306，平均朔望月（2000/1/7日为朔日，JD=5.592804594993741）
function getShuoMonthNo(jd) {
