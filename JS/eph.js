...
/****************************************
以下是天文计算部分,包含有：
物件 SunMoonRiseFallObject   : 用来计算日月的升起、中天、降落
...
var SunMoonRiseFallObject = {
    ...
    Scoord: function (jd, xm, r) {
        var z = new Array(Sun_Moon_Ephemeris.E_Lon((jd + this.dt) / 36525, 5) + Math.PI - 20.5 / rad, 0, 1); //太阳坐标(修正了光行差)
        z = llrConv(z, this.E); //转为赤道坐标
        r.H = rad2rrad(pGST(jd, this.dt) + this.L - z[0]); //得到此刻天体时角
        if (xm == 10 || xm == 1)
    ...
    calcRTS: function (jd, n, Jdl, Wdl, sq) {
        let i, c, r;
        //console.log("calcRTS():" +jd+ "/" +n+ "/" + Jdl+ "/"+Wdl+ "/"+sq+ "/");
        if (!this.rts.length) {
            for (i = 0; i < 31; i++)
        ...
        for (i = -1; i <= n; i++) {
            if (i >= 0 && i < n) { //太阳
                ...
                this.rts[i].s = JD.getTimeStr(r.s - sq); //升
                this.rts[i].z = JD.getTimeStr(r.z - sq); //中
                this.rts[i].j = JD.getTimeStr(r.j - sq); //降
                this.rts[i].c = JD.getTimeStr(r.c - sq); //晨
                this.rts[i].h = JD.getTimeStr(r.h - sq); //昏
                this.rts[i].ch = JD.getTimeStr(r.h - r.c - 0.5); //光照时间,getTimeStr()内部+0.5,所以这里补上-0.5
                this.rts[i].sj = JD.getTimeStr(r.j - r.s - 0.5); //昼长
            }
            r = SunMoonRiseFallObject.Mt(jd + i + sq); //月亮
            c = int2(r.s - sq + 0.5) - jd;
            if (c >= 0 && c < n)
                this.rts[c].Ms = JD.getTimeStr(r.s - sq);
            c = int2(r.z - sq + 0.5) - jd;
            if (c >= 0 && c < n)
                this.rts[c].Mz = JD.getTimeStr(r.z - sq);
            c = int2(r.j - sq + 0.5) - jd;
            if (c >= 0 && c < n)
                this.rts[c].Mj = JD.getTimeStr(r.j - sq);
        }
        this.rts.dn = n;
    }
};
...
var msc = {
    ...
    calc: function (T, L, fa, high) {
        ...
        //视半径
        //this.mRad = Sun_Moon_Ephemeris.moonRad(this.mR,this.mDW);  //月亮视半径(角秒)
        ...
        this.mIll = Sun_Moon_Ephemeris.moonIll(T); //月亮被照面比例
        //中心食计算
    ...
    toHTML: function (fs) {
        ...
        s += '平太阳 ' + JD.getTimeStr(this.pty) + ' 真太阳 <font color=#FF0000>' + JD.getTimeStr(this.zty) + '</font><br>';
...
var ysPL = {
    ...
    lecMax: function (jd) {
        this.lT = new Array();
        for (let i = 0; i < 7; i++)
            this.lT[i] = 0; //分别是:食甚,初亏,复圆,半影食始,半影食终,食既,生光
        this.sf = 0;
        this.LX = '';
        jd = Sun_Moon_Ephemeris.MS_aLon_t2(Math.floor((jd - 4) / 29.5306) * Math.PI * 2 + Math.PI) * 36525; //低精度的朔(误差10分钟),与食甚相差10分钟左右
        var g = new Object(), G = new Object(), u, v;
        //求极值(平均误差数秒)
        u = -18461 * Math.sin(0.057109 + 0.23089571958 * jd) * 0.23090 / rad; //月日黄纬速度差
        v = (Sun_Moon_Ephemeris.M_v(jd / 36525) - Sun_Moon_Ephemeris.E_v(jd / 36525)) / 36525; //月日黄经速度差
        this.lecXY(jd, G);
...
var rsGS = {
    ...
    init: function (jd, n) {
        if (getShuoMonthNo(jd) == getShuoMonthNo(this.Zjd) && this.Zs.length == n * 9)
            return;
        this.Zs.length = 0;
        this.Zjd = jd = Sun_Moon_Ephemeris.MS_aLon_t2(getShuoMonthNo(jd) * Math.PI * 2) * 36525; //低精度的朔(误差10分钟)
    ...
    jieX: function (jd) {
        ...
        for (i = 0; i <= N; i++, t += dt) {
            ...
            if (p[1] != 100 && n4 == 0 || p[1] == 100 && n4 == 1) { //从无交点跳到有交点或反之
                ...
                var dj;
                if (n4 == 0)
                    dj = ls.R2, ls = ls.B; //首坐标
                else
                    dj = ls.R1, ls = ls.A; //末坐标
                ls[2] = 0;
...
var rsPL = {
    ...
    secMax: function (jd, L, fa, high) {
        var i;
        for (i = 0; i < 5; i++)
            this.sT[i] = 0; //分别是:食甚,初亏,复圆,食既,生光
        this.LX = ''; //类型
        this.sf = 0; //食分
        this.sf2 = 0; //食分(日出食分)
        this.sf3 = 0; //食分(日没食分)
        ...
        for (i = 0; i < 5; i++) {
            if (this.sT[i] < this.sun_s || this.sT[i] > this.sun_j)
                this.sT[i] = 0; //升降时间之外的日食算值无效，因为地球不是透明的
        }
        this.sun_s -= dt_T(jd);
        this.sun_j -= dt_T(jd);
    },
